    ...
    where Registry: self::Registry,
{
    ...
    pub fn output(&mut self, config: config::Output) -> Result<BoxedOutputDecl> {
        let factory = Registry::output(&config.kind)
            .ok_or_else(|| anyhow!("Unknown output type: {}", config.kind))?;

        let decl = factory.produce(config.config, self)
            .with_context(|| format!("Failed to build output: (type={})",
                                     config.kind))?;
    ...
    where Registry: self::Registry,
{
    fn input<V>(&mut self, config: config::Input) -> Result<InputHandle<V>>
        where V: InputValue,
    {
        let decl = self.scene.input(&config.input)
    ...
    where Registry: self::Registry,
{
        ...
        where V: DynAttrValue + DeserializeOwned,
    {
        match config {
            config::Attr::Attr { kind, config } => {
                let factory = Registry::free_attr(&kind)
                    .ok_or_else(|| anyhow!("Unknown attribute type: {}", kind))?;

                let decl = factory.produce(config, self)
                    .with_context(|| format!("Failed to build attribute: (type={}) @{}",
                                             kind, name))?;
        ...
        where V: DynAttrValue + DeserializeOwned + Bounded,
    {
        match config {
            config::Attr::Attr { kind, config } => {
                let factory = Registry::bound_attr(&kind)
                    .ok_or_else(|| anyhow!("Unknown attribute type: {}", kind))?;
...
{
    fn node(&mut self, name: &str, config: config::Node) -> Result<NodeHandle<BoxedNodeDecl>> {
        let factory = Registry::node(&config.kind)
            .ok_or_else(|| anyhow!("Unknown node type: {}", config.kind))?;

        let decl = factory.produce(config.config, self)
            .with_context(|| format!("Failed to build node: {} (type={}) @{}",
...
impl<Registry> OutputBuilder for Builder<Registry>
    where Registry: self::Registry,
{
