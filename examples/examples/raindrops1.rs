...
async fn main() -> Result<!, Error> {
    ...
    let raindrops_violet = scene.node(
        "raindrops:violet",
        RaindropsNodeDecl {
            rate: 0.3_f64.fixed(),
            color: (
    ...
    let raindrops_orange = scene.node(
        "raindrops:orange",
        RaindropsNodeDecl {
            rate: 0.3_f64.fixed(),
            color: (
    ...
    let raindrops_iceblue = scene.node(
        "raindrops:iceblue",
        RaindropsNodeDecl {
            rate: 0.3_f64.fixed(),
            color: (
    ...
    let switch_raindrops = scene.node(
        "raindrops",
        SwitchNodeDecl {
    ...
    let (main, registry) = scene.run(switch_raindrops, ConsoleOutputDecl { waterfall: true })?;
