...
trait AsBoxedNodeDecl<Element> {
    fn materialize(self: Box<Self>, size: usize, builder: &mut NodeBuilder) -> Result<BoxedNode<Element>, Error>;
}

impl<T, Element> AsBoxedNodeDecl<Element> for T
    where T: NodeDecl<Element=Element>,
          T::Target: 'static {
    fn materialize(self: Box<Self>, size: usize, builder: &mut NodeBuilder) -> Result<BoxedNode<Element>, Error> {
        return T::materialize(*self, size, builder)
            .map(BoxedNode::wrap);
    }
}
...
impl<Element> BoxedNodeDecl<Element> {
    pub fn wrap<Decl>(decl: Decl) -> Self
        where Decl: NodeDecl<Element=Element> + 'static {
        return Self {
            decl: Box::new(decl),
        };
    }
