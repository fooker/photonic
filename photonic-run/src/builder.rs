...
use crate::registry::{NodeRegistry, OutputRegistry, UnboundAttrRegistry, BoundAttrRegistry};
...
impl Builder {
    ...
    pub fn node(&mut self, name: &str, config: config::Node) -> Result<NodeHandle<boxed::BoxedNodeDecl<color::RGBColor>>, Error> {
        let decl = NodeRegistry::manufacture(&config.kind, config.config, self)
            .context(format!("Failed to build node: {} (type={}) @{}", config.name, config.kind, name))?;
        return self.scene.node(&config.name, decl);
    }

    pub fn unbound_attr<V>(&mut self, name: &str, config: config::Attr) -> Result<boxed::BoxedUnboundAttrDecl<V>, Error>
        where V: UnboundAttrFactory {
        match config {
            ...
            config::Attr::Fixed(value) => {
                V::make_fixed(self, value)
            }
        }
    }

    pub fn bound_attr<V>(&mut self, name: &str, config: config::Attr) -> Result<boxed::BoxedBoundAttrDecl<V>, Error>
        where V: BoundAttrFactory {
        match config {
            config::Attr::Attr { kind, config } => {
    ...
    pub fn output(&mut self, config: config::Output) -> Result<boxed::BoxedOutputDecl<color::RGBColor>, Error> {
