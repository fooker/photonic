...
pub enum Attribute {
    Fixed(f64),
    Dynamic { name: String, value: Arc<Mutex<DynamicValue>> },
}
...
impl Attribute {
    ...
    pub fn get(&self) -> f64 {
        match self {
            &Attribute::Fixed(v) => v,
            &Attribute::Dynamic { ref name, ref value } => value.lock().unwrap().value(),
        }
    }
...
impl Dynamic for Attribute {
    fn update(&mut self, duration: &Duration) {
        match self {
            ...
            &mut Attribute::Dynamic { ref name, ref mut value } => value.lock().unwrap().update(duration),
        }
    }
}
